...
const syncronize = async (
  ...
  const translatedVideos = await Promise.all(
    filteredApiVideos.map(async (video) => ({
      ...video,
      visible: true,
      url_iframe_download_video_audio: buildUrlIframeDownloadVideoAudio(video.video_url),
      download_subtitles: buildSubtitleDownloadUrl(video.video_url, video.channel.targetLanguage),
