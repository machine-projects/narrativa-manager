...
const getAllRegistredVideosObjectKey = async (channel_id) => {
...
const videosToUpdateAndInsert = async (
  ...
  const allvideosChannelSync = await getAllRegistredVideosObjectKey(channel.channelId);
  ...
  for (const video of videos) {
    const videoCadastrado = allvideosChannelSync[video.url];
    if (videoCadastrado) {
...
const channelToDict = (channel) =>{
  const mydict = {}
   channel.forEach(el =>{
    mydict[el.channelId] = el
