...
import { getStartOfDayISOString, getEndOfDayISOString } from "../helper"
...
const fetchVideosFromChannel = async (channelId, maxResults, startDate = null, endDate = null) => {
    try {
        ...
        if (startDate) {
            filter.publishedAfter = getStartOfDayISOString(startDate);
        }
        if (endDate) {
            filter.publishedBefore = getEndOfDayISOString(endDate);
        }
