 ...
 */
async updateChannelAppliedVideosFromVideo(videoId) {
  // Obtém o vídeo correspondente ao ID
  const video = await this.db.collection("video").findOne({ _id: new ObjectId(videoId) });
  if (!video) {
